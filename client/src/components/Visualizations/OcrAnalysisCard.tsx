    ...
    <div className="w-full bg-slate-900 border border-slate-800 rounded-lg p-5 shadow-lg text-slate-100">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
        <div className="flex items-center gap-2">
          <FileText className="text-brand-primary h-5 w-5 animate-pulse" />
        ...
        <div className="flex items-center gap-2 bg-slate-850 px-2.5 py-1 rounded border border-slate-700 text-[11px]">
          ...
          <span className="text-emerald-400 font-bold">{(data.translationConfidence * 100).toFixed(0)}% Confidence</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        <div className="bg-slate-950 p-4 rounded-lg border border-slate-800">
          ...
          <p className="text-sm font-medium leading-relaxed text-slate-300 font-kannada select-all break-words">
            {data.rawKannada}
          </p>
        </div>
        ...
        <div className="bg-slate-950 p-4 rounded-lg border border-slate-800">
          ...
          <p className="text-sm font-semibold leading-relaxed text-slate-200 italic select-all break-words">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="space-y-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider flex items-center gap-1">
              <User size={10} className="text-blue-400" /> Accused / Suspects
            </span>
            <div className="text-xs font-bold text-slate-200">
              {data.entities.suspects.length > 0 ? (
                data.entities.suspects.map((s, i) => (
                  <span key={i} className="inline-block bg-blue-500/10 text-blue-300 border border-blue-500/25 px-1.5 py-0.5 rounded text-[11px]">
                    {s}
                  </span>
                ))
              ) : (
          ...
          <div className="space-y-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider flex items-center gap-1">
              <MapPin size={10} className="text-emerald-400" /> Locations
            </span>
            <div className="text-xs font-bold text-slate-200">
              {data.entities.locations.length > 0 ? (
                data.entities.locations.map((l, i) => (
                  <span key={i} className="inline-block bg-emerald-500/10 text-emerald-300 border border-emerald-500/25 px-1.5 py-0.5 rounded text-[11px]">
                    {l}
                  </span>
                ))
              ) : (
          ...
          <div className="space-y-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider flex items-center gap-1">
              <DollarSign size={10} className="text-amber-400" /> Monetary Amounts
            </span>
            <div className="text-xs font-bold text-slate-200">
              {data.entities.monetaryAmount !== 'None' ? (
                <span className="inline-block bg-amber-500/10 text-amber-300 border border-amber-500/25 px-1.5 py-0.5 rounded text-[11px]">
          ...
          <div className="space-y-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider flex items-center gap-1">
              <AlertCircle size={10} className="text-red-400" /> Weapons
            </span>
            <div className="text-xs font-bold text-slate-200">
              {data.entities.weaponsMentioned && data.entities.weaponsMentioned[0] !== 'None' ? (
                <span className="inline-block bg-red-500/10 text-red-300 border border-red-500/25 px-1.5 py-0.5 rounded text-[11px]">
