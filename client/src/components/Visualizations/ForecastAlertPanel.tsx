      ...
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
            <MapPin size={14} className="text-slate-500" /> {forecast.district}
          </h3>
        </div>
        <Shield size={20} className={styles.badge.includes('red') ? 'text-red-400' : styles.badge.includes('orange') ? 'text-orange-400' : 'text-yellow-400'} />
      </div>

      {/* Predicted Crime Type */}
      <div className="mb-3">
        <span className="text-xs text-slate-400">Predicted Crime:</span>
        <span className="text-sm font-semibold text-white ml-2">{forecast.predicted_crime_type}</span>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <div className="border border-slate-800/60 rounded-md overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800/40 transition"
          >
            <span className="flex items-center gap-1.5">
              <Brain size={12} className="text-blue-400" /> Reasoning
            </span>
            {expanded === 'reasoning' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </button>
          {expanded === 'reasoning' && (
            <div className="px-3 pb-3 text-xs text-slate-400 border-t border-slate-800/40 pt-2">
              {forecast.reasoning}
            </div>
          )}
        </div>
        ...
        <div className="border border-slate-800/60 rounded-md overflow-hidden">
          ...
          >
            <span className="flex items-center gap-1.5">
              <Lightbulb size={12} className="text-amber-400" /> Recommended Action
            </span>
        ...
        <div className="border border-slate-800/60 rounded-md overflow-hidden">
          ...
          >
            <span className="flex items-center gap-1.5">
              <Database size={12} className="text-emerald-400" /> Data Sources ({forecast.data_sources.length})
            </span>
            {expanded === 'sources' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </button>
          {expanded === 'sources' && (
            <div className="px-3 pb-3 border-t border-slate-800/40 pt-2 flex flex-wrap gap-1.5">
              {forecast.data_sources.map((src, i) => (
                <span
                  key={i}
                  className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium"
                >
                  {src}
                </span>
              ))}
...
export const ForecastAlertPanel: React.FC<ForecastAlertPanelProps> = ({ forecasts }) => {
  ...
  return (
    <div className="card-panel rounded-lg p-6 text-slate-200 w-full my-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-800 pb-4 mb-6 gap-3">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            <AlertTriangle className="text-brand-primary" size={20} /> Predictive Intelligence Alerts
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">AI-generated crime forecasts and recommended deployments</p>
        </div>
