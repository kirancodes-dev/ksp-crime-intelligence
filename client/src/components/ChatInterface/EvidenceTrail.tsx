...
const LLM_LABELS: Record<string, { name: string; badge: string; color: string }> = {
  ...
  groq: { name: 'Groq LPU · Llama 3.3 70B', badge: '⚡', color: 'text-orange-400 bg-orange-500/10 border-orange-500/20' },
  gemini: { name: 'Google Gemini 2.5 Flash', badge: '✦', color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
  ollama: { name: 'Ollama · Gemma 2B (Local)', badge: '💻', color: 'text-purple-400 bg-purple-500/10 border-purple-500/20' },
  live: { name: 'Cloud LLM', badge: '☁️', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' },
  mock: { name: 'Rule-based (No LLM)', badge: '🔧', color: 'text-slate-400 bg-slate-500/10 border-slate-500/20' },
  fallback: { name: 'Fallback Mode', badge: '⚠️', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
};
...
export const EvidenceTrail: React.FC<EvidenceTrailProps> = ({ sources, llmMode }) => {
  ...
  return (
    <div className="mt-2 rounded-lg border border-slate-800/60 bg-slate-900 overflow-hidden max-w-full">
      ...
      {/* Expanded Content */}
      {isExpanded && (
        <div className="px-3 pb-3 border-t border-slate-800/40 space-y-3 max-h-[320px] overflow-y-auto">

          {/* LLM Provider Badge */}
          <div className="flex items-center gap-2 pt-3">
            <Cpu size={13} className="text-brand-primary shrink-0" />
            <span className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider">AI Engine:</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider">Analysis Tool:</span>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider">Data Sources Queried:</span>
            </div>
          ...
          <div className="pt-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider">AI Reasoning Path:</span>
            </div>
              ...
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-2">
                  ...
                  <span className={`text-[10px] font-medium leading-tight ${
                    i === steps.length - 1 ? 'text-emerald-400' : 'text-slate-400'
                  }`}>
                    {step}
                  </span>
                </div>
          ...
          {safeSource.description && (
            <div className="flex items-start gap-2 pt-1">
              ...
              <span className="text-[10px] text-slate-400 italic leading-relaxed">{safeSource.description}</span>
            </div>
          )}
        </div>
      )}
